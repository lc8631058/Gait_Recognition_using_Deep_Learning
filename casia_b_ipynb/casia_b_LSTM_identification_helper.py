...
def generate_degree_normalized_list(gallery_path, images):
    ...
    for image in images[0]:
        image_path = os.path.join(gallery_path, images[1], image)
        if image[-6:] == '55.png':
            list_55.append(((imread(image_path)/255).round(decimals=3)).flatten())
        elif image[-6:] == '65.png':
            list_65.append(((imread(image_path)/255).round(decimals=3)).flatten())
        elif image[-6:] == '75.png':
            list_75.append(((imread(image_path)/255).round(decimals=3)).flatten())
        elif image[-6:] == '85.png':
            list_85.append(((imread(image_path)/255).round(decimals=3)).flatten())
    list_total = []
